...
if ($GenerateReport) {
    ...
    # コードメトリクスの生成
    Write-Host "📏 コードメトリクスを生成中..." -ForegroundColor Cyan
    dotnet tool install --global dotnet-metrics --version 1.0.0
    dotnet metrics collect --project src/BrowserSelector.App --output quality-report/metrics.json
...
# 警告の確認
Write-Host "⚠️ ビルド警告を確認しています..." -ForegroundColor Yellow
...
# コード品質のサマリー
Write-Host "`n📋 コード品質分析サマリー:" -ForegroundColor Green
...
if ($warnings) {
    Write-Host "  - 警告: ⚠️ 検出" -ForegroundColor Yellow
} else {
    Write-Host "  - 警告: ✅ なし" -ForegroundColor Green
}

Write-Host "`n🎉 コード品質分析が完了しました!" -ForegroundColor Green
