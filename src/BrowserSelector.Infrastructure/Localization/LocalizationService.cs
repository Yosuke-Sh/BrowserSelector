...
public class LocalizationService : ILocalizationService
{
    ...
    {
        _logService?.LogTrace($"GetString呼び出し: {key}, 現在のカルチャ: {CurrentCulture.Name}, カスタムリソース数: {_customResources.Count}, JSONリソース数: {_jsonResources.Count}", "LocalizationService");

        // カスタム言語リソースを優先
        if (_customResources.TryGetValue(key, out string? customValue))
        {
            _logService?.LogTrace($"カスタムリソースから取得: {key} = {customValue}", "LocalizationService");
            return customValue;
        }
        ...
        {
            _logService?.LogTrace($"JSONリソースから取得: {key} = {jsonValue}", "LocalizationService");
            return jsonValue;
        }
        ...
        {
            _logService?.LogTrace($"デフォルトリソースから取得: {key} = {fallbackValue}", "LocalizationService");
    ...
    private async Task LoadJsonResourcesAsync(string cultureCode)
    {
        try
        {
            ...
            if (stream == null)
            {
                _logService?.LogTrace($"JSONリソースファイルが見つかりません: {resourceName}", "LocalizationService");
                return;
            }
            ...
            if (languageFile?.Resources != null)
            {
                ...
                _logService?.LogTrace($"JSONリソースを読み込みました: {cultureCode} ({languageFile.Resources.Count}個のリソース)", "LocalizationService");
            }
        }
    ...
    private async Task LoadCustomLanguageResourcesAsync(string cultureCode)
    {
        try
        {
            ...
            if (customResources != null)
            {
                ...
                _logService?.LogTrace($"カスタム言語リソースを読み込みました: {cultureCode} ({customResources.Count}個のリソース)", "LocalizationService");
            }
            else
            {
                _logService?.LogTrace($"カスタム言語リソースが見つかりません: {cultureCode}", "LocalizationService");
            }
        }
        catch (ArgumentException ex)
        {
